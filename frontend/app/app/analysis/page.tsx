...
function buildLiquidityProjection(data: AnalysisLatestResponse) {
  ...
  const avgInflow =
    collection_health.inflow_count > 0
      ? collection_health.total_inflow / collection_health.inflow_count
      : 0;
  const avgDaysBetween = Math.max(1, Math.round(collection_health.avg_days_between));
...
export default function AnalysisPage() {
  ...
  const effectiveData = data ?? (isDemo ? getMockAnalysisData(displayCount) : null);
  const isMockData = isDemo && !data;
  ...
  const expenseData = useMemo(
    () => (effectiveData ? expenseChartData(effectiveData.expense_breakdown) : []),
    [effectiveData]
  );
  const minThreshold = effectiveData ? effectiveData.liquidity.current_balance * 0.2 : 0;
  const zeroDateStr =
    effectiveData && effectiveData.liquidity.runway_days < 90
      ...
      <div dir={dir} className="flex flex-col h-full">
        <div className="flex-1 flex items-center justify-center p-6">
          <Card className="max-w-md w-full text-center p-8">
            ...
            <Button asChild><Link href="/app/import">{isAr ? "ارفع ملف CSV" : "Upload CSV"}</Link></Button>
          </Card>
        </div>
      </div>
    );
            ...
            <CardContent className="p-6">
              <div className="flex flex-wrap items-center gap-6 justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-3xl font-bold tabular-nums">{summary.health_score}</p>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    {isAr ? `الرصيد يكفي ${liquidity.runway_days} يوم` : `Balance covers ${liquidity.runway_days} days`}
                  </span>
                  <span className="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">
                    {isAr ? `إجمالي المشاكل: ${summary.total_problems}` : `Total problems: ${summary.total_problems}`}
                  </span>
                  <span className="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">
