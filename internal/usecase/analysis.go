...
// AnalysisUseCase runs financial analysis on tenant transactions and generates Arabic recommendations.
type AnalysisUseCase struct {
	repo domain.AnalysisRepository
}

// NewAnalysisUseCase returns an AnalysisUseCase that uses the given repository.
func NewAnalysisUseCase(repo domain.AnalysisRepository) *AnalysisUseCase {
...
func (uc *AnalysisUseCase) RunAnalysis(ctx context.Context, tenantID uuid.UUID) (*domain.CashAnalysis, error) {
...
func analyzeRecurring(txns []domain.TransactionForAnalysis) []domain.RecurringPayment {
	...
	for cp, list := range byCounterparty {
		...
		// Use average amount (outflows as positive for display)
