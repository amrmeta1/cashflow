...
// fakeAnalysisRepo implements domain.AnalysisRepository for tests.
...
func TestLiquidityAnalysis_Critical(t *testing.T) {
	...
	now := time.Now()
	// Last 30 days: 30 outflows of 10k each = 300k total -> daily burn 10k. Last balance 100k -> runway 10 days (< 15).
	var txns []domain.TransactionForAnalysis
	balance := 400000.0 // start high
	for i := 0; i < 30; i++ {
	...
	// last balance = 400k - 300k = 100k, runway = 100k/10k = 10 days
	txns[29].RunningBalance = 100000

	repo := &fakeAnalysisRepo{txns: txns}
	uc := NewAnalysisUseCase(repo)
...
func TestLiquidityAnalysis_Healthy(t *testing.T) {
	tenantID := uuid.MustParse("10000000-0000-0000-0000-000000000001")
	now := time.Now()
	// Last 30 days outflow 300k (10k/day). Last balance 1M -> runway 100 days (> 30).
	var txns []domain.TransactionForAnalysis
	for i := 0; i < 30; i++ {
		d := now.AddDate(0, 0, -29+i)
		txns = append(txns, domain.TransactionForAnalysis{
...
func TestExpenseBreakdown_DominantCategory(t *testing.T) {
	tenantID := uuid.MustParse("10000000-0000-0000-0000-000000000001")
	now := time.Now()
	// Outflows: A=60, B=25, C=15 (total 100). A > 50% -> IsDominant.
	var txns []domain.TransactionForAnalysis
	balance := 500.0
...
func TestCollectionHealth_Irregular(t *testing.T) {
	tenantID := uuid.MustParse("10000000-0000-0000-0000-000000000001")
	base := time.Now()
	// Two inflows 40 days apart -> LargestGapDays 40 -> IsIrregular.
	txns := []domain.TransactionForAnalysis{
...
func TestHealthScore_Calculation(t *testing.T) {
	tenantID := uuid.MustParse("10000000-0000-0000-0000-000000000001")
	now := time.Now()
	// Target: liquidityScore=50 (warning), collectionScore=100, expenseScore=100.
	// HealthScore = 50*0.4 + 100*0.3 + 100*0.3 = 20+30+30 = 80.
	// Warning runway: 15 <= runway < 30. Balance 200k, burn 10k/day -> 20 days.
	// Two inflows same day -> gap 0 -> CollectionScore 100. Outflows spread so no dominant -> expenseScore 100.
